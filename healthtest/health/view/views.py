...
def Alldocs(request):
    docs = Doctor.objects.order_by('-workplace')
    context={
        'doctors': docs
    }
...
def information(request):
    ...
    try:
        ...
        try:
            ...
            utype = "Doctor"
            #doc=Doctor.objects.get(username=uname)
...
def appointments(request):
    ...
    try:
        ...
        appointments = Appointment.objects.filter(patient=p, rate='0')
...
def createAppt(request):
    ...
    try:
        ...
        utype = "Patient"
        # Patients can create an appointment with any Doctor
        hospitals=Hospital.objects.order_by("-hosp_city")
...
def indexview(request):
    if 'Doctor' in request.POST:
        return render(request,basic.html/loginp)
    elif 'Patient' in request.POST:
    return render(request,basic.html/logind)
'''
...
def createPresInfo(request):
    ...
    global uname
    doctor = Doctor.objects.get(username=uname)
    app = Appointment.objects.get(patient=pat, done="0")
    ...
    city = doctor.workplace.hosp_city
    diseases = (request.POST['disease'])
...
def nomore(request, pat_id):
    ...
    patient.sactive = "0"
    patient.save()
    stat = forstats.objects.get(for_patient=p)
    stat.dead="1"
    stat.save()
    # diseses = 'cold'
    ...
    city = stat.current_city
    total_death = forstats.objects.values('death').count()
    total_death_disease_wise = forstats.objects \
        .values('disease') \
        .filter(Q(current_city=p_city) & Q(disease=p_disease) & Q(month=month))\
        .annotate(DeathCount=Count('death'))\
        .order_by('-disease')

    total_death_disease_wise_count=total_death_disease_wise.count()
    ...
    if total_death_disease_wise > threepercentoftotal:
        ...
        for entry in total_death_disease_wise:
            categories.append(entry['disease'])
            survived_series.append((float(entry['DeathCount']) / float(total_count)) * 100)

        info="City:"+city+"& Disease:"+city
        ...
        return  render(request,'HealthNet/home.html',{
            'error_message':'Prediction not possible'
        })
